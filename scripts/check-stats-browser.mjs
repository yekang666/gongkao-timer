...
try {
  ...
  assert(mockRecordFields.score && mockRecordFields.total && mockRecordFields.questions && mockRecordFields.correct, `Mock custom record fields are incorrect: ${JSON.stringify(mockRecordFields)}`);
  ...
  await evaluate(`window.__recordsBeforeCreate = window.state.records; document.querySelector('#createRecordModule').value='写作'; document.querySelector('#createRecordModule').dispatchEvent(new Event('change')); document.querySelector('#createRecordMinutes').value='25'; document.querySelector('#createRecordTotalScore').value='50'; document.querySelector('#createRecordScore').value='32'; document.querySelector('#recordCreateForm').requestSubmit();`);
  const createdEssayRecord = await evaluate(`(() => { const record=window.state.records.find(item => item.module === '写作'); return { module:record?.module, totalScore:record?.totalScore, score:record?.score, questions:record?.questions, correct:record?.correct, dialogOpen:document.querySelector('#recordCreateDialog').open }; })()`);
  assert(createdEssayRecord.module === '写作' && createdEssayRecord.totalScore === 50 && createdEssayRecord.score === 32 && createdEssayRecord.questions === null && createdEssayRecord.correct === null && !createdEssayRecord.dialogOpen, `Created essay record fields are incorrect: ${JSON.stringify(createdEssayRecord)}`);
  await evaluate(`window.state.records = window.__recordsBeforeCreate; localStorage.setItem('examTimer.records.v1', JSON.stringify(window.__recordsBeforeCreate)); document.querySelector('#statsBtn').click();`);
  await evaluate(`window.__recordsBeforeEssaySpeed = [...window.state.records]; import('./js/speed.js').then(module => { Object.assign(window.state, { mode:'single', elapsed:120, startedAt:new Date(Date.now() - 120000).toISOString(), status:'paused', laps:[], lastLapElapsed:0 }); module.finishSpeedSession(); });`);
  await evaluate(`[...document.querySelectorAll('#singleModulePicker .module-choice')].find(button => button.querySelector('strong')?.textContent === '写作').click();`);
  const essaySpeedFields = await evaluate(`({ total:!document.querySelector('#speedTotalScoreWrap').classList.contains('hidden'), score:!document.querySelector('#speedScoreWrap').classList.contains('hidden'), questions:document.querySelector('#speedCountWrap').classList.contains('hidden'), correct:document.querySelector('#speedCorrectWrap').classList.contains('hidden'), label:document.querySelector('#speedScoreLabel').textContent })`);
  assert(essaySpeedFields.total && essaySpeedFields.score && essaySpeedFields.questions && essaySpeedFields.correct && essaySpeedFields.label === '本题得分', `Essay speed fields are incorrect: ${JSON.stringify(essaySpeedFields)}`);
  await evaluate(`document.querySelector('#speedTotalScore').value='50'; document.querySelector('#speedScore').value='32'; document.querySelector('#nextSpeedStepBtn').click(); document.querySelector('#skipTrainingMetaBtn').click();`);
  const savedEssaySpeed = await evaluate(`(() => { const record=window.state.records.find(item => item.module === '写作'); return { totalScore:record?.totalScore, score:record?.score, questions:record?.questions, correct:record?.correct }; })()`);
  assert(savedEssaySpeed.totalScore === 50 && savedEssaySpeed.score === 32 && savedEssaySpeed.questions === null && savedEssaySpeed.correct === null, `Essay speed record is incorrect: ${JSON.stringify(savedEssaySpeed)}`);
  await evaluate(`window.state.records = window.__recordsBeforeEssaySpeed; localStorage.setItem('examTimer.records.v1', JSON.stringify(window.__recordsBeforeEssaySpeed)); document.querySelector('#statsBtn').click();`);
  ...
  const xingceEditorFields = await evaluate(`({ total:!document.querySelector('#editRecordTotalScore'), score:!document.querySelector('#editRecordScore'), questions:Boolean(document.querySelector('#editRecordQuestions')), correct:Boolean(document.querySelector('#editRecordCorrect')) })`);
