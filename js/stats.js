...
function renderStats() {
  ...
  const modules = TRACKING_CATEGORIES; $('#moduleStats').innerHTML = modules.map(name => {
    ...
    return `<div class="module-row"><strong>${name}</strong><span>${analytics.rows.length ? (timedRows.length ? formatDuration(avg) : '已录入复盘') : '暂无记录'}${scoreText}${avgPerQuestion ? ` / 题均 ${formatClock(avgPerQuestion).slice(3)}` : ''}${accuracyText}</span></div>`;
  }).join('');
